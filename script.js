...
/* ─── LOCALSTROAGE CACHE YARDIMÇILARI ───────────────── */
const CACHE_PREFIX  = 'ghcache_';
const CACHE_TTL_MS  = 5 * 60 * 1000; // 5 dəqiqə

function cacheSet(folder, files) {
  try {
    localStorage.setItem(CACHE_PREFIX + folder, JSON.stringify({ ts: Date.now(), files }));
  } catch (_) {}
}

function cacheGet(folder) {
  try {
    const raw = localStorage.getItem(CACHE_PREFIX + folder);
    if (!raw) return null;
    const { ts, files } = JSON.parse(raw);
    if (Date.now() - ts > CACHE_TTL_MS) return null; // köhnəlibsə ignore et
    return files;
  } catch (_) { return null; }
}

function cacheGetStale(folder) {
  // TTL fərq etmədən — yalnız xəta zamanı fallback üçün
  try {
    const raw = localStorage.getItem(CACHE_PREFIX + folder);
    if (!raw) return null;
    return JSON.parse(raw).files || null;
  } catch (_) { return null; }
}

async function githubFetchFiles(folder) {
  // 1. Keçərli cache varsa, birbaşa qaytar
  ...
  try {
    ...
    if (!res.ok) {
      // API xətası — köhnə cache-dən istifadə et
      const stale = cacheGetStale(folder);
      return stale || [];
    }
    ...
    const result = files.filter(f => f.type === 'file').map(f => ({ name: f.name, download_url: f.download_url }));
    cacheSet(folder, result); // uğurlu nəticəni cache-ə yaz
    return result;
  } catch (_) {
    // Şəbəkə xətası — köhnə cache-dən istifadə et
    const stale = cacheGetStale(folder);
    return stale || [];
  }
}
...
/* ─── BÖLMƏ ÜÇÜN YÜKLƏMƏ İNDİKATORU ───────────────── */
function setSectionLoading(el) {
  if (!el) return;
  el.innerHTML = `<div class="loading-indicator"><div class="loading-spinner"></div><p>Yüklənir...</p></div>`;
}

function setSectionEmpty(el, icon, title, text) {
  if (!el) return;
  el.innerHTML = `<div class="empty-section"><div class="empty-icon">${icon}</div><h2>${title}</h2><p>${text}</p></div>`;
}

/* ─── GITHUB KONTENTİ YÜKLƏYİCİ (Paralel & Müstəqil) ── */
async function loadGitHubContent() {
  // Bütün bölmələr eyni anda, bir-birindən asılı olmayaraq yüklənir
  ...
  try {
    ...
    if (imgFiles.length > 0) {
      ...
      imgFiles.forEach((f, i) => {
        ...
        // Şəkili əvvəlcə ayrıca yüklə — uğurlu olduqda src-i set et
        ...
        tempImg.onerror = () => {
          ...
          img.src = '';
          item.style.display = 'none'; // pozulmuş şəkili gizlə
        };
        tempImg.src = f.download_url;

        item.appendChild(img);
        item.addEventListener('click', () => openLightbox(f.download_url, 'image'));
      ...
      setSectionEmpty(albumEl, '[◉°]', 'Albom', 'Hələlik şəkil yüklənməyib.');
    }
  } catch (_) {
    const stale = cacheGetStale('images');
    if (stale && stale.filter(f => /\.(jpg|jpeg|png|gif|webp)$/i.test(f.name)).length > 0) {
      const imgFiles = stale.filter(f => /\.(jpg|jpeg|png|gif|webp)$/i.test(f.name)).reverse();
      ...
      setSectionEmpty(albumEl, '[◉°]', 'Albom', 'Şəkillər yüklənə bilmədi. Sonra yenidən cəhd edin.');
    }
  }
}

/* ─── MƏKTUBLAR BÖLMƏSİ ─────────────────────────────── */
async function loadLettersSection() {
  const lettersEl = document.getElementById('letters-content');
  if (!lettersEl) return;

  setSectionLoading(lettersEl);
  ...
  try {
    ...
    if (txtFiles.length > 0) {
      const loaded = await Promise.allSettled(txtFiles.map(async f => {
      ...
      const validLetters = loaded
        .filter(r => r.status === 'fulfilled')
        .map(r => r.value);

      lettersEl.innerHTML = validLetters.map(l => {
      ...
      setSectionEmpty(lettersEl, 'જ⁀➴', 'Məktublar', 'Burada sizin səmimi sözləriniz olacaq.');
    }
  } catch (_) {
    setSectionEmpty(lettersEl, 'જ⁀➴', 'Məktublar', 'Məktublar yüklənə bilmədi. Sonra yenidən cəhd edin.');
  }
}
...
async function loadMusicSection() {
  ...
  if (musicEl) setSectionLoading(musicEl);
