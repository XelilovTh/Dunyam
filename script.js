...
async function loadGitHubContent() {
  try {
    const response = await fetch('/.netlify/functions/get-github-data');
    const allData = await response.json();

    if (!Array.isArray(allData)) throw new Error("Məlumat massiv deyil");

    // --- 1. ŞƏKİLLƏR (IMAGES) ---
    const imgFiles = allData.filter(f => f.path.startsWith('images/') && /\.(jpg|jpeg|png|gif|webp)$/i.test(f.name)).reverse();
    ...
    // --- 2. MƏKTUBLAR (LETTERS) ---
    const txtFiles = allData.filter(f => f.path.startsWith('letters/') && /\.txt$/i.test(f.name)).reverse();
    ...
    // --- 3. MUSİQİ (MUSIC) ---
    const audioFiles = allData.filter(f => f.path.startsWith('music/') && /\.(mp3|ogg|wav|m4a|aac|flac)$/i.test(f.name)).reverse();
